...
def DFG_python(root_node,index_to_code,states):
    ...
    if (len(root_node.children)==0 or root_node.type=='string') and root_node.type!='comment':        
        ...
        if root_node.type==code:
            ...
            return [(code,idx,'comesFrom',[code],states[code].copy())],states  # from last statement to this statement
        else:
            if root_node.type=='identifier':
                states[code]=[idx]
            return [(code,idx,'comesFrom',[],[])],states  # tag now states
    elif root_node.type in def_statement:
        name=root_node.child_by_field_name('name')  # function name
        value=root_node.child_by_field_name('value')  # TODO: unknown type
        DFG=[]
        if value is None:
        ...
        for node in right_nodes:
            temp,states=DFG_python(node,index_to_code,states)  # for right node to get DFG and states 
            DFG+=temp
...
def DFG_python_with_ACTION(root_node,index_to_code,states):
    assignment=['assignment','augmented_assignment','for_in_clause']
    if_statement=['if_statement']
    for_statement=['for_statement']
    while_statement=['while_statement']
    do_first_statement=['for_in_clause'] 
    def_statement=['default_parameter']
    operators = ['+', '-', '*', '/', '%', '>>', '<<', '>>>', '&', '|', '^', '&&', '||', '==', '!=', '>', '<', '>=', '<=', 'in', 'instanceof', 'delete', '!', '~', '&&', '||', '+=', '-=', '*=', '/=', '%=', '<<=', '>>=', '>>>=', '&=', '^=', '|=']
    states=states.copy() 
    if (len(root_node.children)==0 or root_node.type=='string') and root_node.type!='comment':        
        idx,code=index_to_code[(root_node.start_point,root_node.end_point)]
        if root_node.type==code:
        ...
        if value is None:
            ...
            name_indexs=tree_to_variable_index(name,index_to_code)
            value_indexs=tree_to_variable_index(value,index_to_code)
            temp,states=DFG_python_with_ACTION(value,index_to_code,states)
            DFG+=temp            
            for index1 in name_indexs:
                idx1,code1=index_to_code[index1]
                for index2 in value_indexs:
        ...
        if root_node.type=='for_in_clause':
            ...
            if len(right_nodes)==0:
                right_nodes=[root_node.child_by_field_name('right')]
        DFG=[]
        # 1. add action to states 2. add action to temp 3. extra return action
        functional_action = []
        for node in right_nodes:
            temp,states=DFG_python_with_ACTION(node,index_to_code,states)  # for right node to get DFG and states 
            DFG+=temp
            code = []
            if node.type == 'call':
                for child in node.children:
                    if child.type == 'identifier':
                        code = index_to_code[(child.start_point,child.end_point)]
                        code = [code]
                        break
                        # indexs=tree_to_variable_index(child,index_to_code)
                        # for index in indexs:
                        #     idx,code=index_to_code[index]
                        #     DFG.append((code,idx,'action',[],[]))
                        #     states[code]=[idx]
            functional_action.append(code)
        ...
        for left_node,right_node in zip(left_nodes,right_nodes):
            ...
            for token1_index in left_tokens_index:
                ...
                for token2_index in right_tokens_index:
                    ...
                    if code2 in operators or code2 in functional_action:
                        depends_on.append((code2,idx2))
                        break
                    depends_on.append((code2,idx2))
                temp.append((code1,idx1,'computedFrom',[index_to_code[x][1] for x in right_tokens_index],
                             [index_to_code[x][0] for x in right_tokens_index]))
        ...
        for child in root_node.children:
            if 'else' in child.type:
                tag=True
            if child.type not in ['elif_clause','else_clause']:
                temp,current_states=DFG_python_with_ACTION(child,index_to_code,current_states)
                DFG+=temp
            else:
                temp,new_states=DFG_python_with_ACTION(child,index_to_code,states)
                DFG+=temp
                others_states.append(new_states)
        others_states.append(current_states)
        if tag is False:
        ...
        for i in range(2):
            ...
            if len(right_nodes)==0:
                right_nodes=[root_node.child_by_field_name('right')]
            for node in right_nodes:
                temp,states=DFG_python_with_ACTION(node,index_to_code,states)
                DFG+=temp
            for left_node,right_node in zip(left_nodes,right_nodes):
                left_tokens_index=tree_to_variable_index(left_node,index_to_code)
                right_tokens_index=tree_to_variable_index(right_node,index_to_code)
                ...
                DFG+=temp   
            if  root_node.children[-1].type=="block":
                temp,states=DFG_python_with_ACTION(root_node.children[-1],index_to_code,states)
                DFG+=temp 
        dic={}
        for x in DFG:
            if (x[0],x[1],x[2]) not in dic:
        ...
        for i in range(2):
            for child in root_node.children:
                temp,states=DFG_python_with_ACTION(child,index_to_code,states)
                DFG+=temp    
        dic={}
        for x in DFG:
            if (x[0],x[1],x[2]) not in dic:
